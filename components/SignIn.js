  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.result === true) {
          Toast.success("Connexion success !", "top", { duration: 2000 });
          dispatch(login({ username: data.username, email: data.email, token: data.token, id: data.id }));
          onClose();
        } else {
          Toast.error("Connection failed !", "top", { duration: 2000 });
        }
      } else {
        console.error("❌ (SignIn): Error in response of request", response);
        return;
      }
    } catch (err) {
