...
export default function FavouriteScreen() {
  ...
  const placesLiked = placesLikedList.map((place, i) => { // Boucle pour afficher les lieux likés
    return (
  ...
  return (
    <View style={styles.container}> {/* View contenant les éléments de la page */}
      <Header title="My Favorites" showInput={false} /> {/* Affichage du composant Header */}
      <View style={styles.favouritesScreenContainer}> {/* View contenant les différents éléments de la page */}
        <TouchableOpacity style={styles.button}> {/* Bouton pour planifier sa journée */}
          <Text style={styles.textButton}>Plan my day !</Text>
        </TouchableOpacity>
        <ScrollView showsVerticalScrollIndicator={false}>{placesLiked}</ScrollView> {/* Affichage des lieux likés */}
      </View>
    </View>
  );
}
